...
def new_rain_file(new_rain):
  ...
  save_new_rain_total(new_rain, new_rain)
...
def get_rain_total():
  ...
  try:
    ...
    print "Yearly Rain Total: Last Update:{:s} Total:{:s}".format(*rain_data)
    total_rain_in = float(rain_data[-1])
  except IOError as err:
    print "Unable to open rain file: {:s}".format(err)
  ...
  return total_rain_in, current_year

def save_new_rain_total(total_rain_in, new_rain):
  ...
  # build our new value
  data_string = "{:s},{:.2f}\n".format(timenow.strftime("%Y-%m-%d %H:%M:%S.%f"), total_rain_in)
...
if __name__ == '__main__':
  ...
  hostname = config.get('FENCE_STATION', 'host')
  port = config.getint('FENCE_STATION', 'port')
  authkey = config.get('FENCE_STATION', 'authkey')
  ...
  while True:
    ...
    if current_year != timenow.year:
      ...
      new_rain_file(interval_rain)
      current_year = timenow.year
    else:
      # if we got some rain, add it to the file
      if interval_rain_in > 0.0:
        save_new_rain_total(total_rain_in, interval_rain_in)
