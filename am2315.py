...
VERSION = "1.0.20141229a"

class AM2315(object):
  """A Python2 class to read the AOSONG 2315 sensor."""
  ...
  def get_readings(self):
    ...
    t_c, t_f = self._calculate_temperature(t_msb, t_lsb)
  ...
  def _read_raw(self):
    ...
    # even though 4 bytes were requested, 8 are returned
    count, data = self.pi.i2c_read_device(self.handle, 8)
...
def main():
  ...
  while True:
    ...
    print("{:s}:  T(F): {:.2f}   T(C): {:.1f}   RH: {:.1f}".format(timenow, t_f, t_c, rh))
